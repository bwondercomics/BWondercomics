...
describe("reader feed and latest panels", () => {
  ...
  it("sorts posts, sanitizes content, toggles entries, and falls back on missing images", async () => {
    ...
    items[0].querySelector(".feed-item-toggle")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
  ...
  it("renders empty and failure feed states", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({ posts: [] })));
  ...
  it("wires module feed previews and open-close behavior", async () => {
    ...
    container.querySelector(".pb-feed-toggle")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    ...
    container.querySelector(".pb-feed-exit")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
  ...
  it("disables the module dropdown feed when the contract says so", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({ post: getContractFixture("latestPost") })));
  ...
  it("wires the legacy right-panel feed mode against the live reader DOM", async () => {
    ...
    initRightPanelFeed();
    document.getElementById("latestHeadingBtn")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    ...
    document.getElementById("feedExitBtn")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
  ...
  it("renders latest previews with truncation, fallback images, and optional media links", () => {
    ...
    const preview = latestPreviewHtml("<p>Hello <strong>world</strong> from <custom-embed>bad</custom-embed> the feed.</p>", 12);
    ...
    expect(latestBody.querySelector(".latest-link--left")?.getAttribute("href")).toBe("feed.html#latest");
