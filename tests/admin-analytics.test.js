...
describe('admin analytics', () => {
  ...
  it('renders pages-read summaries, ratio cards, and visitor history', async () => {
    ...
    expect(document.getElementById('analyticsInsight')?.textContent).toContain('18 pages read');
    ...
    expect(document.getElementById('analyticsReferrersList')?.textContent).toContain('google.com');
    expect(document.getElementById('analyticsBrowsersList')?.textContent).toContain('Chrome');
    expect(document.getElementById('analyticsDevicesList')?.textContent).toContain('Mobile');
    expect(document.getElementById('analyticsEventsList')?.textContent).toContain('cta-click');
    ...
    expect(document.getElementById('readsOverTimeTotals')?.textContent).toContain('pages read');
  });
  ...
  it('reloads reads-over-time when chart controls change', async () => {
    ...
    expect(
      fetchMock.mock.calls.some(([url]) =>
        String(url).includes(
          '/api/admin/analytics/reads-over-time?range=7d&entry_key=battle-bros%3A10'
        )
      )
    ).toBe(true);
    expect(document.getElementById('readsOverTimeTotals')?.textContent).toContain('6 pages read');
    expect(
      document.getElementById('readsOverTimeEntryShell')?.classList.contains('is-hidden')
    ).toBe(false);
  });
  ...
  it('renders visitor history activity chips with readable units', async () => {
    ...
    const visitorListText =
      document.getElementById('analyticsVisitorHistoryList')?.textContent || '';
