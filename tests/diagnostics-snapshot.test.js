...
function legacyDiagnosticsPayload(generatedAt) {
  return {
    ...
    backups: {
      ...
      items: [
        { name: "files-20260101-010101.tar.gz", path: "var/backups/files.tar.gz", size: 2048, modifiedAt: generatedAt },
        { name: "db-20260101-010101.sql", path: "var/backups/db.sql", size: 1024, modifiedAt: generatedAt },
      ],
    },
    serviceStatus: {
      generatedAt,
      process: { pid: 1234, uptimeSeconds: 3600 },
      system: { platform: "Linux", python: "3.12.0", disk: { totalBytes: 10_000, freeBytes: 4_000 } },
    },
    testStatus: {
      status: "failed",
      available: true,
...
describe("admin diagnostics snapshot", () => {
  beforeEach(() => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url, options = {}) => {
      ...
      if (String(url).includes("/api/admin/diagnostics/snapshot")) {
        return {
          ok: true,
          json: async () => buildSnapshot(new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString()),
        };
      }
  ...
  it("falls back to legacy diagnostics endpoints when snapshot routes are unavailable", async () => {
    ...
    expect(document.getElementById("diagnosticsBanner").textContent).toContain("backend refresh route is not available yet");
  });
