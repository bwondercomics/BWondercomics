  ...
  beforeEach(() => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url) => {
      ...
      if (path.endsWith("/api/session")) {
        return { ok: true, json: async () => ({ user: { email: "admin@example.com", role: "admin" } }) };
      }
      if (path.endsWith("/api/admin/diagnostics/snapshot")) {
        return {
          ...
          json: async () => ({
            ...
            serviceStatus: { items: [{ id: "api", label: "API", status: "ok", summary: "online" }] },
          }),
        };
      }
      ...
      if (path.endsWith("/api/admin/ops/history")) {
        return {
          ...
          json: async () => ({
            runs: [
              { id: "1", label: "Queue", status: "queued", startedAt: new Date().toISOString(), userEmail: "a", output: "" },
              { id: "2", label: "Run", status: "running", startedAt: new Date().toISOString(), userEmail: "a", output: "" },
              { id: "3", label: "Done", status: "completed", startedAt: new Date().toISOString(), userEmail: "a", output: "" },
              { id: "4", label: "Fail", status: "failed", startedAt: new Date().toISOString(), userEmail: "a", output: "" },
            ],
          }),
        };
      }
      if (path.endsWith("/api/admin/ops/backups")) {
        return {
          ...
          json: async () => ({
            ...
            files: [{ name: "files.tar.gz", createdAt: new Date().toISOString(), sizePretty: "2 KB" }],
          }),
        };
      }
