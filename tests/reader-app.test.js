...
import {
  ...
  mountReaderDom,
  stubReaderGlobals,
} from "./helpers/reader-fixture.js";
...
function buildReaderEntryData() {
  ...
  const entries = Object.fromEntries(
    Object.entries(fixture.entries).map(([name, pages]) => [
      name,
      pages.map((page) =>
        page.startsWith("protected/")
          ? `/api/protected/${page.replace(/^protected\//, "")}`
          : page,
      ),
    ]),
  );
...
describe("reader app bootstrap", () => {
  ...
  it("boots the reader against the live markup contract and restores saved progress", async () => {
    ...
    expect(mocks.loadPageConfigWithFallback).toHaveBeenCalledWith(expect.any(Function), "battle-bros");
