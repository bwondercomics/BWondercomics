...
- Layout: the builder uses a full-page shell with a top toolbar, one collapsible side panel, a central live canvas viewport, and an admin-only overlay layer above the iframe for live hover/selection chrome. The side-panel collapsed/expanded choice persists in `localStorage`.
