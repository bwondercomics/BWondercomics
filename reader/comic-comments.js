...
(() => {
  ...
  function getCurrentTargetId() {
    ...
    const seriesId =
      (typeof getActiveSeriesId === 'function' ? getActiveSeriesId() : 'battle-bros') ||
      'battle-bros';
  ...
  function applyAuthMode(ctx) {
    ...
    if (ctx.authToggle) {
      ctx.authToggle.textContent = isLogin
    ...
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent('bbSessionChanged', { detail: { user: ctx.user || null } })
        );
      } catch {
