...
function createUsersManager({ hideAllSections, setActiveNav } = {}) {
  ...
  function setEmailListStatus(message, isError = false) {
    ...
    el.emailListStatus.style.background = isError ? 'var(--danger)' : 'var(--success)';
    el.emailListStatus.style.color = isError ? 'var(--text)' : 'var(--bg-dark)';
  }
  ...
  function setPremiumCodesStatus(message, isError = false) {
    ...
    el.premiumCodesStatus.style.background = isError ? 'var(--danger)' : 'var(--success)';
    el.premiumCodesStatus.style.color = isError ? 'var(--text)' : 'var(--bg-dark)';
  }
    ...
    users.forEach((u) => {
      ...
      item.innerHTML = `
        <div class="entry-info">
          <div class="entry-name">${escapeHtml(u.displayName || u.email || 'User')}</div>
          <div class="entry-meta" style="opacity:0.9;">${escapeHtml(u.email || '')}</div>
          <div class="entry-meta" style="opacity:0.8;">Role: ${escapeHtml(role)}</div>
          <div class="entry-meta" style="opacity:0.75;">${escapeHtml(emailStatus)}</div>
          <div class="entry-meta" style="opacity:0.7;">${escapeHtml(premiumStatus)}</div>
        </div>
  ...
  function renderPremiumCodes(codes = []) {
    ...
    codes.forEach((code) => {
      ...
      const status = code.active ? (redeemedCount > 0 ? 'Active' : 'Unused') : 'Inactive';
      ...
      metaParts.push(`Redeemed ${redeemedCount} time${redeemedCount === 1 ? '' : 's'}`);
