...
function createPageBuilder({ sanitizeSeriesId, getActiveSeriesId, hideAllSections, setActiveNav }) {
  ...
  let activeSectionInsertIndex = null;
  ...
  function getEditorWidth(mode, sidebarMode = getEffectiveSidebarMode()) {
    ...
    const baseWidth = mode === "docked"
      ? (navCollapsed ? 620 : 520)
      : (navCollapsed ? 420 : 320);
    const sidebarBonus = sidebarMode === "collapsed"
  ...
  function ensureCleanWorkspace(message) {
    ...
    if (dirtyScope === "section") {
      setCanvasStatus(message || "Save or discard the current section settings before switching.", "warning");
      renderCanvas();
      return false;
    }
  ...
  function toggleEditorMode() {
    ...
    const nextMode = band === "wide"
      ? (currentMode === "docked" ? "collapsed" : "docked")
      : (currentMode === "collapsed" ? "overlay" : "collapsed");
  ...
  async function updatePublishState(isPublished) {
    ...
    if (!ensureCleanWorkspace("Save or discard your current changes before updating publish state.")) {
      return;
    }
    ...
    try {
      ...
      releaseButtons();
      setEditorStatus(isPublished ? "Failed to publish changes." : "Failed to save draft.", "danger");
      renderEditorPanel();
    }
  }
  ...
  async function moveModuleToTarget(moduleId, targetSectionId, targetColumnIndex, insertIndex) {
    ...
    sourceSection.modules = (sourceSection.modules || []).filter((module) => module.id !== moduleId);
    ...
    const sourceOrderedIds = sortModulesForColumn(sourceSection, sourceColumnIndex)
      .map((module) => module.id);
  ...
  async function insertSectionAt(index) {
    ...
    const sectionIds = currentPage.sections.map((section) => section.id).filter((id) => id !== newSection.id);
  ...
  function renderModuleInsertBar(sectionId, columnIndex, insertIndex) {
    const isActive = activeInsertTarget
  ...
  function renderCanvas() {
    ...
    const html = `
      ...
      ${sections.map((section, sectionIndex) => {
        ...
        const sectionDraft = isSettingsOpen && activeSectionDraft
          ? activeSectionDraft
          : {
    ...
    el.pbCanvas.querySelectorAll('[data-action="toggle-section-settings"]').forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (dirtyScope === "section" && activeSectionId !== sectionId) {
          setCanvasStatus("Save or discard the current section settings before switching sections.", "warning");
          renderCanvas();
          return;
        }
    ...
    if (activeSectionId) {
      ...
      el.pbCanvas.querySelector('[data-action="discard-section-settings"]')?.addEventListener("click", () => {
      ...
      el.pbCanvas.querySelector('[data-action="save-section-settings"]')?.addEventListener("click", async () => {
    ...
    el.pbCanvas.querySelectorAll('[data-action="toggle-module-picker"]').forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const isSameTarget = activeInsertTarget
  ...
  async function saveActiveModuleDraft() {
    ...
    const updated = await updateModule(selectedModule.id, { config: cloneValue(activeModuleDraft) });
    if (!updated) {
  ...
  function renderEditorPanel() {
    ...
    if (activeEditorTab === "theme") {
      ...
      title = selectedModule ? `${getModuleLabel(selectedModule.moduleType)} Module` : "Choose a Module";
    ...
    el.pbModuleEditor.querySelectorAll(".pb-editor-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        if (!ensureCleanWorkspace("Save or discard your current changes before switching inspector tabs.")) {
          renderEditorPanel();
          return;
        }
  ...
  function initPageBuilder() {
    ...
    if (sidebar) {
      sidebar.querySelectorAll(".pb-sidebar-tab").forEach((tab) => {
        tab.addEventListener("click", () => {
          sidebar.querySelectorAll(".pb-sidebar-tab").forEach((button) => button.classList.remove("active"));
    ...
    el.pbAddPage?.addEventListener("click", async () => {
      ...
      if (!slug) return;
      const title = prompt("Enter page title:", slug.replace(/-/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()));
