...
function getModuleSummary(moduleType, config) {
  switch (moduleType) {
    ...
    case "text":
      return config.content?.replace(/<[^>]*>/g, "").trim().slice(0, 90) || "Rich text block";
    case "image":
...
function bindPromoDraftEvents({
  ...
  el.pbModuleEditor.querySelectorAll(".pb-promo-item").forEach((itemEl) => {
    ...
    itemEl.querySelector('[data-action="move-up"]')?.addEventListener("click", () => {
      if (index <= 0) return;
      const nextConfig = normalizePromoConfig(config);
      [nextConfig.items[index - 1], nextConfig.items[index]] = [nextConfig.items[index], nextConfig.items[index - 1]];
      commit(nextConfig, true);
    });
    ...
    itemEl.querySelector('[data-action="move-down"]')?.addEventListener("click", () => {
      ...
      const nextConfig = normalizePromoConfig(config);
      [nextConfig.items[index], nextConfig.items[index + 1]] = [nextConfig.items[index + 1], nextConfig.items[index]];
      commit(nextConfig, true);
    });
...
function bindSocialDraftEvents({
  ...
  el.pbModuleEditor.querySelectorAll(".pb-social-item").forEach((itemEl) => {
    ...
    itemEl.querySelector('[data-action="move-up"]')?.addEventListener("click", () => {
      if (index <= 0) return;
      const nextConfig = normalizeSocialConfig(config);
      [nextConfig.buttons[index - 1], nextConfig.buttons[index]] = [nextConfig.buttons[index], nextConfig.buttons[index - 1]];
      commit(nextConfig, true);
    });
    ...
    itemEl.querySelector('[data-action="move-down"]')?.addEventListener("click", () => {
      ...
      const nextConfig = normalizeSocialConfig(config);
      [nextConfig.buttons[index], nextConfig.buttons[index + 1]] = [nextConfig.buttons[index + 1], nextConfig.buttons[index]];
      commit(nextConfig, true);
    });
  ...
  el.pbModuleEditor.querySelectorAll(".pb-social-style-input, .pb-social-style-color, .pb-social-style-range").forEach((input) => {
...
function bindGenericModuleDraftEvents({
  ...
  const syncDraft = () => {
    setDraftConfig(collectGenericModuleDraft(el.pbModuleEditor, draftConfig || selectedModule.config || {}));
    markDirty("module");
  };
...
export function renderModuleEditorContent({ currentPage, selectedModuleId, draftConfig = null }) {
  ...
  switch (moduleType) {
    case "header":
      contentSections.push(renderSectionCard(
        "Content",
        "Header Copy",
