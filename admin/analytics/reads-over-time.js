...
import { formatStat, getCssVar } from './shared.js';
...
function createReadsOverTimeAnalytics() {
  ...
  function updateReadsOverTimeEntryOptions(payload) {
    ...
    const validEntries = entryViews.filter((item) => item?.displayNumber != null).slice(0, 50);
    ...
    validEntries.forEach((item) => {
      ...
      duplicateDisplayNumbers.set(displayKey, (duplicateDisplayNumbers.get(displayKey) || 0) + 1);
    ...
    validEntries.forEach((item) => {
      ...
      const label =
        needsSeriesContext && item.seriesTitle ? `${item.seriesTitle} · ${baseLabel}` : baseLabel;
      option.title = label;
    ...
    if (current && validValues.includes(current)) {
      ...
      el.readsOverTimeEntry.value = String(
        validEntries[0].entryKey || validEntries[0].displayNumber
      );
    }
  }
